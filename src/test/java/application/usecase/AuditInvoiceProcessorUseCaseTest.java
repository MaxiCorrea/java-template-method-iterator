...
class AuditInvoiceProcessorUseCaseTest {
	...
	InvoiceIterator iterator = new InvoiceIterator() {
		...
		private final Iterator<Invoice> data = List.of(
				...
				new Invoice("3", BigDecimal.TWO, true)
				).iterator();
	...
	@Test
	void shouldProcessAllInvoicesAndSumAmounts() {
		AuditInvoiceProcessorUseCase usecase = new AuditInvoiceProcessorUseCase();
		ProcessingResult result = usecase.process(iterator);
		assertEquals( new ProcessingResult(3, new BigDecimal("22")), result);
	}
